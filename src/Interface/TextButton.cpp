...
void Meatball::TextButton::drawText() {
    ...
    Meatball::drawText(*config->font, config->font->baseSize, text.c_str(), rect.x+rect.width*0.5, rect.y+rect.height*0.5, hovered? config->hoveredTextColor : config->textColor);
...
void Meatball::TextButton::setText(const std::string &_newText) {
    ...
    int textWidth = Meatball::measureTextWidth(*config->font, config->font->baseSize, text.c_str());

    while (textWidth > rect.width) {
        text.erase(text.end());
        textWidth = Meatball::measureTextWidth(*config->font, config->font->baseSize, text.c_str());
    }
}